...
public class ChatAdapter extends IChatAdapter {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, final int position) {
        ...
        if (mRecycleView.getChatTimeBubbleId() > 0) {
            chatHolder.chatTime.setBackgroundResource(mRecycleView.getChatTimeBubbleId());
        }
        ...
        if (msg.getMsgType() >= MessageInfo.MSG_TYPE_TIPS) {
            ...
            if (mRecycleView.getTipsMessageBubbleId() > 0) {
                tipsHolder.tips.setBackgroundResource(mRecycleView.getTipsMessageBubbleId());
            }
        ...
        if (msg.isSelf()) {
            if (chatHolder.dataView != null) {
                if (mRecycleView.getSelfBubbleId() > 0) {
                    chatHolder.dataView.setBackgroundResource(mRecycleView.getSelfBubbleId());
                } else {
            ...
            if (chatHolder.dataView != null) {
                if (mRecycleView.getOppositeBubbleId() > 0) {
                    chatHolder.dataView.setBackgroundResource(mRecycleView.getOppositeBubbleId());
                } else {
