...
public class MessageInfoUtil {
    ...
    public static final int MESSAGE_PK_INVITE = 103; // PK邀请
    public static final int MESSAGE_PK_PICK = 104; // PK
    ...
    public static MessageInfo buildCustomMessage(String action, String message) {
        MessageInfo info = new MessageInfo();
        TIMMessage TIMMsg = new TIMMessage();
        TIMCustomElem ele = new TIMCustomElem();
        ele.setData(action.getBytes());
        ...
        if (timMessage.getElementCount() > 0) {
            ...
            if (ele == null) {
                if (BuildConfig.DEBUG)
                    QLog.e(MessageInfoUtil.class.getSimpleName(), "msg found null elem");
                return null;
            }

            TIMElemType type = ele.getType();
            if (type == TIMElemType.Invalid) {
                if (BuildConfig.DEBUG) QLog.e(MessageInfoUtil.class.getSimpleName(), "invalid");
                return null;
            }

            if (type == TIMElemType.Custom) { //自定义消息
                if (BuildConfig.DEBUG) TUIKit.m(MessageInfoUtil.class, "TIMElemType.Custom");
                ...
                if (data.equals(GROUP_CREATE)) {
                    ...
                    msgInfo.setExtra(new String(customElem.getExt()));
                } else if (getType(msgInfo.getTIMMessage()) == MESSAGE_PK_INVITE) {
                    msgInfo.setMsgType(MessageInfo.MSG_TYPE_PK_INVITE);
                    msgInfo.setExtra(new String(customElem.getExt()));
                } else if (getType(msgInfo.getTIMMessage()) == MESSAGE_PK_PICK) {
                    msgInfo.setMsgType(MessageInfo.MSG_TYPE_PK_PICK);
                    msgInfo.setExtra(new String(customElem.getExt()));
                } else {
                    msgInfo.setExtra("[自定义消息]");
                }
            } else if (type == TIMElemType.GroupTips) {//组消息
                if (BuildConfig.DEBUG) TUIKit.m(MessageInfoUtil.class, "TIMElemType.GroupTips");
                ...
                if (BuildConfig.DEBUG) TUIKit.m(MessageInfoUtil.class, "TIMElemType.其他的");
                if (type == TIMElemType.Text) {
                    ...
                    if (txtEle.getIndex() < 1 || txtEle.getData() == null) {
                        if (BuildConfig.DEBUG)
                            QLog.e(MessageInfoUtil.class.getSimpleName(), "txtEle data is null or index<1");
                        return null;
                    }
            ...
            if (BuildConfig.DEBUG)
                QLog.e(MessageInfoUtil.class.getSimpleName(), "msg elecount is 0");
            return null;
        }
