...
public class SessionManager implements UIKitRequestHandler, TIMRefreshListener, UIKitMessageRevokedManager.MessageRevokeHandler {
    ...
    public void init() {
        ...
        mTopList = new HashSet<String>(mSessionPreferences.getStringSet(TOP_LIST, new HashSet<String>()));  //必须新new一下，因为SharedPreferences在put时判断引用相同不提交
        ...
        for (int j = 0; j < infos.size(); j++) {
            ...
            for (int i = 0; i < dataSource.size(); i++) {
                ...
                if (cacheInfo.getPeer().equals(update.getPeer())) {
                    update.setTitle(cacheInfo.getTitle());
                    update.setIconUrl(cacheInfo.getIconUrl());
                    if(update.getLastMessage()!=null&&cacheInfo.getLastMessage()!=null){
                        MessageInfo oldLastMsg = cacheInfo.getLastMessage();
                        MessageInfo newLastMsg = update.getLastMessage();
                        if(oldLastMsg.getMsgId()!=null&&oldLastMsg.getMsgId().equals(newLastMsg.getMsgId())){
                            newLastMsg.setStatus(oldLastMsg.getMsgType());  //因为这里客户端接收到对方撤回消息通知后，设置了消息的类型为撤回类型，但下次服务端返回相同消息。类型还不是撤回，这里要设置一下
                        }
                    }
                    dataSource.remove(i);
    ...
    public Object handleRequest(UIKitRequest msg) {
        ...
        //刷新会话列表，目前C2C消息撤回有用到
            ...
            updateMessageRevoked(msg.getRequest().toString());
        }
        //其他模块的会话跳转
    ...
    public void updateMessageRevoked(String msgId) {
        if(mProvider == null){
            return;
        }

        for (int i = 0; i < mProvider.getDataSource().size(); i++) {
            SessionInfo sessionInfo = mProvider.getDataSource().get(i);
            if (sessionInfo.getLastMessage().getMsgId().equals(msgId)) {
                sessionInfo.getLastMessage().setMsgType(MessageInfo.MSG_STATUS_REVOKE);
                sessionInfo.getLastMessage().setStatus(MessageInfo.MSG_STATUS_REVOKE);
    ...
    public void handleInvoke(TIMMessageLocator locator) {
        if (mProvider != null) {
            for (int i = 0; i < mProvider.getDataSource().size(); i++) {
                SessionInfo sessionInfo = mProvider.getDataSource().get(i);
                MessageInfo lastMsg = sessionInfo.getLastMessage();
                if(checkLastMsgIsInvoke(locator, lastMsg)){
                    return;
                }
            }
        }
    ...
    private boolean checkLastMsgIsInvoke(TIMMessageLocator locator, MessageInfo lastMsg) {
        if (lastMsg != null) {
            TIMMessageExt ext = new TIMMessageExt(lastMsg.getTIMMessage());
            if (ext.checkEquals(locator)) {
                lastMsg.setMsgType(MessageInfo.MSG_STATUS_REVOKE);
                lastMsg.setStatus(MessageInfo.MSG_STATUS_REVOKE);
                mProvider.updateAdapter();
                updateMessageRevoked(lastMsg.getMsgId());
                return true;
            }
        }
        return false;
