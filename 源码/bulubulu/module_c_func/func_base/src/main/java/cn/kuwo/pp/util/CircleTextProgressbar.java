...
public class CircleTextProgressbar extends androidx.appcompat.widget.AppCompatTextView {
     ...
     * 外部轮廓的颜色。
     */
    private int outLineColor = Color.parseColor("#1AFFE43F");
     ...
     * 进度条的颜色。
     */
    private int progressLineColor = Color.parseColor("#FFFFE43F");

    /**
     * 进度条的宽度。
     */
    private int progressLineWidth = UtilsCode.INSTANCE.dp2px(10);
    ...
    private void initialize(Context context, AttributeSet attributeSet) {
        ...
        typedArray.recycle();
        outLineColor = context.getResources().getColor(R.color.progressbar_bg);
        txtPaint = getPaint();
        txtPaint.setColor(getCurrentTextColor());
        txtPaint.setAntiAlias(true);
        txtPaint.setTextAlign(Paint.Align.CENTER);
        txtPaint.setTypeface(Typeface.createFromAsset(getResources().getAssets(), "fonts/Akrobat-kuwo-Bold.ttf"));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(outLineWidth+progressLineWidth);
        mPaint.setColor(outLineColor);
        canvas.drawCircle(bounds.centerX(), bounds.centerY(), outerRadius - (outLineWidth+progressLineWidth) / 2, mPaint);

        //画字
        float textY = bounds.centerY() - (txtPaint.descent() + txtPaint.ascent()) / 2;
        canvas.drawText(getText().toString(), bounds.centerX(), textY, txtPaint);
