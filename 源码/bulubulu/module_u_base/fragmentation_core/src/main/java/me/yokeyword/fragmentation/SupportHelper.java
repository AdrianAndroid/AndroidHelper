...
public class SupportHelper {
    ...
    static List<Fragment> getWillPopFragments(FragmentManager fm, String targetTag, boolean includeTarget) {
        Fragment target = fm.findFragmentByTag(targetTag); // 找到这个Fragment
        List<Fragment> willPopFragments = new ArrayList<>();

        List<Fragment> fragmentList = fm.getFragments(); // 获取所有的Fragment
        if (fragmentList == null) return willPopFragments;

        int size = fragmentList.size();

        int startIndex = -1;// 从这个开始的索引
        for (int i = size - 1; i >= 0; i--) { // 倒叙
    ...
    /**
     * 因为还有FragmentManagerImpl 的情况
     *
     * @param fragmentManager
     * @return
     */
    public static boolean isParentFragmentManager(FragmentManager fragmentManager) {
        return fragmentManager != null && fragmentManager.getClass() == FragmentManager.class;
    }
}
