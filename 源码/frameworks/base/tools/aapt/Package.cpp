...
status_t writeAPK(Bundle* bundle, const String8& outputFile, const sp<OutputSet>& outputSet)
{
     ...
     */
    // Leo:准备Zip文件，如果文件存在，则根据bundle设置的"update"或者"force"进行对应处理
    ...
    // Leo: 先把assets丢进去（前面准备好的东西，包括了assets目录, res目录下values以外的子目录;--因为values已经被编译到资源索引表了(resources.arsc)）
    ...
    // Leo:再把用-j指定的包含classes的jar或者zip包给丢进去
    ...
    // do so here
    // Leo:如果被要求生成依赖文件，则在这里进行，最后写到指定apk输出目录下，如bin/resources.ap_.d
