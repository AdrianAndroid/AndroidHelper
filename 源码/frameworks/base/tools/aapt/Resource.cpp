...
// Leo:对res目录下的各个资源子目录进行处理，会对资源文件名做合法性检查，并将其添加到ResourceTable内
...
status_t buildResources(Bundle* bundle, const sp<AaptAssets>& assets, sp<ApkBuilder>& builder)
{
    ...
    // Leo:String8是android framework源码定义的数据格式，用来保存UTF-8字符的字符串，类似的还有一个String16，用来放UTF-16字符串的。
    ...
    // Leo:初始化并设置一些attribute，比如package, minSdkVersion, uses-sdk
    ...
    // Leo:添加被引用资源包，比如系统的那些android:命名空间下的资源
    ...
    // Leo:处理overlay（重叠包，如果指定的重叠包有和当前编译包重名的资源，则使用重叠包的）
    ...
    while(current.get()) {
        ...
        if (index >= 0) {
            ...
            while ((res=it.next()) == NO_ERROR) {
                ...
                // Leo:上一步添加到资源表的过程中，其实并没有对values资源进行处理，因为values比较特殊，需要经过编译之后，才能添加到资源表中
    ...
    if (table.hasResources()) {
        // Leo:在继续编译其他资源之前，我们需要先给bag资源（attrs，比如orientation这种属性的取值范围定义的子元素）分配id，因为其他资源可能对它们有引用。
    ...
    if (layouts != NULL) {
        ...
        while ((err=it.next()) == NO_ERROR) {
            ...
            // Leo:可以编译xml资源文件了,我们已经为它准备好了一切可能引用到的东西（value, drawable等）
            // Leo:程序会对layouts, anims, animators等逐一调用该方法
            // Leo:编译xml资源文件分为：解析xml文件、赋予属性名称资源id、解析属性值、扁平化为二进制文件
    ...
    // Leo:从bundle读取必要的tag并写到manifestTree，处理package name重载，把各种相对路径的名字改为绝对路径
    err = massageManifest(bundle, &table, manifestTree);
    if (err < NO_ERROR) {
        return err;
    }
    // Leo:编译manifest xml文件
...
// Leo:将规则更新到ProguardKeepSet中，然后打开proguard文件进行写入（proguard文件由-G命令指定）
status_t
