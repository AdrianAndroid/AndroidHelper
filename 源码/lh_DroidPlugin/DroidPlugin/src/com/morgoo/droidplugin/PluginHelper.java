 ...
 * 外观模式，在Application的onCreate方法中进行插件管理（PluginManager）、
 * 进程管理（PluginProcessManager）等模块的初始化
 */
public class PluginHelper implements ServiceConnection {
    ...
    private void initPlugin(Context baseContext) {
        ...
        try {
            ...
            // 插件管理
            try {
                // 当执行 PluginManager.onServiceConnected() 时就会执行 PluginHelper.onServiceConnected()
                PluginManager.getInstance().addServiceConnection(PluginHelper.this);
                // 初始化启动 PluginManagerService
