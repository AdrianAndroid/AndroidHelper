...
public class PluginPackageParser {
    ...
    public PluginPackageParser(Context hostContext, File pluginFile) throws Exception {
        ...
        // 得到自定义的PackageParser的类对象，内部实际通过反射调用的系统PackageParser的对应方法，自定义该类主要是为了版本适配
        mParser = PackageParser.newPluginParser(hostContext);
        // 创建了系统PackageParser类的对象，给插件用的
        // 获取插件的Package对象
        ...
        // 得到插件的 ArrayList<Activity> activities，这个Activity是PackageParser的内部类
