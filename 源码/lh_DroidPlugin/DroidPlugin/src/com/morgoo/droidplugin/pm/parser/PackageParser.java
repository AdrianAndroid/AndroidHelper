 ...
 * 插件解析器，用来解析一个插件apk中的信息。策略模式(抽象类），针对不同sdk版本提供了不同的实现，
 * 最终是通过反射调用系统 android.content.pm 包下的PackageParser来获取插件中的所有信息。
 * 如签名文件，dex路径，清单文件信息（包括声明的四大组件，权限，版本号）等。
 * 【其实获取到一个插件apk对应的packageParser就可以获取到基本所有的信息】，在首次装载完成之后，
 * 会使用一个Map<packageName,PackageParser>进行缓存。
 * 判断一个插件是否已经装载也是根据是否命中缓存来判断。
 */
