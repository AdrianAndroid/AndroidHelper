...
public class PluginManager implements ServiceConnection {
    ...
    public void onServiceConnected(final ComponentName componentName, final IBinder iBinder) {
        // 通过Binder通信，这儿可以直接看成拿的是 IPluginManagerImpl 对象
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    // 调用以下方法：
                    // PluginHelper.onServiceConnected()
                    ...
                    try {
                        // 解锁等待：IActivityManagerHookHandler.handlePendingIntent()
    ...
    public int installPackage(String filepath, int flags) throws RemoteException {
        try {
            if (mPluginManager != null) {
                // 通过Binder 调用服务实现类 IPluginManagerImpl.java
