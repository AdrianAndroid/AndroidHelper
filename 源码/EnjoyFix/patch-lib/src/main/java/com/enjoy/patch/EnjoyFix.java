...
public class EnjoyFix {
    ...
    private static File initHack(Context context) {
        ...
        File hackDir = context.getDir("hack", Context.MODE_PRIVATE);//差分包下的目录
        File hackFile = new File(hackDir, "hack.jar"); // 生成的查分包
        if (!hackFile.exists()) {
    ...
    public static void installPatch(Application application, File patch) {
        File hackFile = initHack(application); //找到这个文件
        ...
        files.add(hackFile);
        File dexOptDir = application.getCacheDir(); // 文件夹（不需要权限）
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) { // N以上
            try {
                // android N 热修复
                ClassLoaderInjector.inject(application, classLoader, files); //CLASS_PRIVATE
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    V23.install(classLoader, files, dexOptDir); // 放到dexElements
                } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    V19.install(classLoader, files, dexOptDir); //4.4以上 // 放到dexElements
                } else {  // >= 14
                    V14.install(classLoader, files, dexOptDir); // 放到dexElements
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
