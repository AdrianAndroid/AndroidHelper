...
public final class ViewParentCompat {
    ...
    public static boolean onStartNestedScroll(ViewParent parent, View child, View target,
        ...
        if (parent instanceof NestedScrollingParent2) { // 要是容器的话
            // First try the NestedScrollingParent2 API
            return ((NestedScrollingParent2) parent).onStartNestedScroll(child, target,
                    nestedScrollAxes, type); // 调用这个容器的onStartNestedScroll
        } else if (type == ViewCompat.TYPE_TOUCH) { //TYPE_TOUCH
    ...
    public static void onNestedPreScroll(ViewParent parent, View target, int dx, int dy,
        ...
        if (parent instanceof NestedScrollingParent2) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                try {
                    parent.onNestedPreScroll(target, dx, dy, consumed); //调用parent的onNestedPreScroll
                } catch (AbstractMethodError e) {
                    Log.e(TAG, "ViewParent " + parent + " does not implement interface "
