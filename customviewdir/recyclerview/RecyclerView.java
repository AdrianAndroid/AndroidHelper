...
public class RecyclerView extends ViewGroup implements ScrollingView,
    ...
    void scrollStep(int dx, int dy, @Nullable int[] consumed) {
        ...
        int consumedY = 0;
        if (dx != 0) {
            consumedX = mLayout.scrollHorizontallyBy(dx, mRecycler, mState); //---> LinearLayout.scrollHorizontallyBy
        }
        if (dy != 0) {
            consumedY = mLayout.scrollVerticallyBy(dy, mRecycler, mState); //---> LinearLayout.scrollVerticallyBy
        }
    ...
    public static class RecycledViewPool {
        ...
        void factorInBindTime(int viewType, long bindTimeNs) {
            ScrapData scrapData = getScrapDataForType(viewType); //创建
    ...
    public final class Recycler {
        ...
        ViewHolder tryGetViewHolderForPositionByDeadline(int position,
            ...
            if (mState.isPreLayout()) {
                holder = getChangedScrapViewForPosition(position); // ---> TODO
                fromScrapOrHiddenOrCache = holder != null;
            }
            // 1) Find by position from scrap/hidden list/cache
            if (holder == null) {
                holder = getScrapOrHiddenOrCachedHolderForPosition(position, dryRun); // --> TODO mChangedScrap 与动画相关
                if (holder != null) {
            ...
            if (holder == null) {
                ...
                if (mAdapter.hasStableIds()) {
                    holder = getScrapOrCachedViewForId(mAdapter.getItemId(offsetPosition), // ---> TODO mAttachedScrap / mCachedViews
                            type, dryRun);
                    if (holder != null) {
                        // update position
                ...
                if (holder == null && mViewCacheExtension != null) { //自定义的，没有实现，不用看
                    // We are NOT sending the offsetPosition because LayoutManager does not
                    // know it.
                    final View view = mViewCacheExtension
                            .getViewForPositionAndType(this, position, type); //---> 自定义缓存
                    if (view != null) {
                ...
                if (holder == null) { // fallback to pool
                    ...
                    holder = getRecycledViewPool().getRecycledView(type); // --> TODO 从缓冲池里面获取
                    if (holder != null) { //找到了
                ...
                if (holder == null) { //没有在缓存中
                    ...
                    holder = mAdapter.createViewHolder(RecyclerView.this, type); // --> TODO createViewHolder
                    if (ALLOW_THREAD_GAP_WORK) {
            ...
            if (mState.isPreLayout() && holder.isBound()) {
                ...
                holder.mPreLayoutPosition = position;
            } else if (!holder.isBound() || holder.needsUpdate() || holder.isInvalid()) { // 显示，绑定
                ...
                final int offsetPosition = mAdapterHelper.findPositionOffset(position);
                bound = tryBindViewHolderByDeadline(holder, offsetPosition, position, deadlineNs); //-->TODO BindViewHolder
            }
        ...
        public void recycleView(@NonNull View view) {
            ...
            recycleViewHolderInternal(holder);///////////-》TODO
        ...
        void recycleCachedViewAt(int cachedViewIndex) {
            ...
            addViewHolderToRecycledViewPool(viewHolder, true); //先缓存起来
            mCachedViews.remove(cachedViewIndex); //然后删除掉
        }
         ...
         */
        void recycleViewHolderInternal(ViewHolder holder) { // -- 处理 CacheView 、RecyclerViewPool 的缓存
            ...
            if (forceRecycle || holder.isRecyclable()) {
                if (mViewCacheMax > 0
                    ...
                    if (cachedViewSize >= mViewCacheMax && cachedViewSize > 0) {  // DEFAULT_SIZE == 2
                    ...
                    mCachedViews.add(targetCacheIndex, holder); //位置改变，在缓存起来
                    cached = true;
                }
                if (!cached) {
                    addViewHolderToRecycledViewPool(holder, true); //新的Holder缓存起来
                    recycled = true;
                }
            } else {
        ...
        void addViewHolderToRecycledViewPool(@NonNull ViewHolder holder, boolean dispatchRecycled) {
            ...
            holder.mOwnerRecyclerView = null;
            getRecycledViewPool().putRecycledView(holder);//添加到缓存池中
        }
        ...
        void scrapView(View view) {
            ...
            if (holder.hasAnyOfTheFlags(ViewHolder.FLAG_REMOVED | ViewHolder.FLAG_INVALID)
                ...
                mAttachedScrap.add(holder); //缓存屏幕内的
            } else {
        ...
        ViewHolder getScrapOrCachedViewForId(long id, int type, boolean dryRun) {
            ...
            final int count = mAttachedScrap.size(); //缓存还在屏幕内的ViewHolder
            for (int i = count - 1; i >= 0; i--) {
                final ViewHolder holder = mAttachedScrap.get(i);
                if (holder.getItemId() == id && !holder.wasReturnedFromScrap()) {
                    if (type == holder.getItemViewType()) { //找到了
                        holder.addFlags(ViewHolder.FLAG_RETURNED_FROM_SCRAP);
                        if (holder.isRemoved()) {
                        ...
                        mAttachedScrap.remove(i); //先删除掉
                        removeDetachedView(holder.itemView, false); //去掉界面
                        quickRecycleScrapView(holder.itemView); //湖南村起来
                    }
                }
            }

            // Search the first-level cache
            final int cacheSize = mCachedViews.size(); //缓存的View
            for (int i = cacheSize - 1; i >= 0; i--) {
                final ViewHolder holder = mCachedViews.get(i);
                if (holder.getItemId() == id && !holder.isAttachedToTransitionOverlay()) {
                    if (type == holder.getItemViewType()) { //找到了
                        if (!dryRun) {
                            mCachedViews.remove(i); //直接删除掉
                        }
                        return holder;
                    } else if (!dryRun) { //没找到
                        recycleCachedViewAt(i);
    ...
    public abstract static class LayoutManager {
         ...
         * @param recycler Recycler to use to recycle child
         */
        public void removeAndRecycleViewAt(int index, @NonNull Recycler recycler) {///////////-》TODO
            final View view = getChildAt(index);
            removeViewAt(index);
            recycler.recycleView(view);///////////-》TODO
        }
        ...
        public void detachAndScrapAttachedViews(@NonNull Recycler recycler) {
            ...
            for (int i = childCount - 1; i >= 0; i--) {
                ...
                scrapOrRecycleView(recycler, i, v); //-->TODO
            }
        }

        private void scrapOrRecycleView(Recycler recycler, int index, View view) {
            final ViewHolder viewHolder = getChildViewHolderInt(view); //获取这个ViewHolder
            if (viewHolder.shouldIgnore()) {
                if (DEBUG) {
            ...
            if (viewHolder.isInvalid() && !viewHolder.isRemoved()
                    && !mRecyclerView.mAdapter.hasStableIds()) { //有效
                removeViewAt(index);
