...
public class NestedScrollingChildHelper {
    ...
    public boolean startNestedScroll(@ScrollAxis int axes, @NestedScrollType int type) {
        if (hasNestedScrollingParent(type)) { //刚开始第一次是false，parent还没有找到
            // Already in progress
            return true;
        }
        if (isNestedScrollingEnabled()) { //外部设置的
            ViewParent p = mView.getParent(); //得到parent
            View child = mView;
            while (p != null) {
                if (ViewParentCompat.onStartNestedScroll(p, child, mView, axes, type)) {
                    setNestedScrollingParentForType(type, p); //根据TYPE设置Parent
    ...
    public boolean dispatchNestedPreScroll(int dx, int dy, @Nullable int[] consumed,
        ...
        if (isNestedScrollingEnabled()) {
            ...
            if (dx != 0 || dy != 0) {
                ...
                consumed[0] = 0;
                consumed[1] = 0;
                ViewParentCompat.onNestedPreScroll(parent, mView, dx, dy, consumed, type); //从Child传过来
