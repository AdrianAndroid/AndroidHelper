...
Java_com_example_ffmpeg_XPlay_Open(JNIEnv *env, jobject thiz, jstring url, jobject surface) {
    ...
    if (!codec) {
        ...
        LOGW("avcodec_find success!");
    }
    LOGW("avcodec_alloc_context3 before!");
    //解码器初始化
    AVCodecContext *vc = avcodec_alloc_context3(codec);
    avcodec_parameters_to_context(vc, ic->streams[videoStream]->codecpar);
    LOGW("avcodec_alloc_context3 after!");

    vc->thread_count = 8;
    //打开解码器
    LOGW("avcodec_open2 before! -->%d,%d,%d", vc->thread_count,vc->width, vc->height);
    re = avcodec_open2(vc, 0, 0);
    LOGW("avcodec_open2 after! -->%d", re);
    ...
    LOGW("avcodec_open2 audio before before!");
    //音频解码器初始化
    AVCodecContext *ac = avcodec_alloc_context3(acodec);
    avcodec_parameters_to_context(ac, ic->streams[audioStream]->codecpar);
    ac->thread_count = 8;
    LOGW("avcodec_open2 audio before!");
    //打开解码器
